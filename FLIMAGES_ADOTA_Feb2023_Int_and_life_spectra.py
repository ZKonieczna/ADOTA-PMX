...
mat_fname=r"\\chem-mh-store\D\Members\Zuzanna Konieczna\20220803_DOTA_MB_2MBR_DOTA_3\Lifetime_Data\LifetimeImageData.mat"
mat_contents = sio.loadmat(mat_fname,squeeze_me=True)
lifetimes=mat_contents['lifetimeImageData']


mat_fname=r"\\chem-mh-store\D\Members\Zuzanna Konieczna\20220803_DOTA_MB_2MBR_DOTA_3\Lifetime_Data\LifetimeAlphaData.mat"
...
sum_int=np.sum(intensities[60:300],axis=0)
plt.imshow(sum_int)
plt.colorbar()
#plt.savefig('ADOTA_3_SI_fig_Int.jpg', dpi=1200)
plt.show()
...
plt.imshow(twod_lifetime, cmap='jet_r', vmin=0, vmax=8) #use to adjust look up table or scale bars
plt.colorbar()
plt.show()
...
m=ax.imshow(twod_lifetime,cmap='plasma_r',vmin=2,vmax=8,alpha=twod_intensities)
fig.colorbar(m)
fig.savefig('ADOTA_3_LF_Masked_SI.jpg',dpi=1200)
plt.show()
...
plt.hist(real_lifteimes, bins = 30,range=[2,10], rwidth=0.9,ec='black',color='darkmagenta',alpha=0.8)
